...
### `AmunEV_Engine_V2.py:105862` — in `<module>()`

```python
'trailer_price':           _f(lambda C: np.where(C['sd'] > 0, C['ao'], np.where(C['sd'] < 0, C['ho'], np.nan)), 'tick', False, 'moneyline', 'what the comeback pays', 'LIVE_TRAILER'),
```
